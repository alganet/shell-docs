...
> the name of the shell or script being run
...
`$0` is the name the shell was invoked as. Its exact value depends on how the shell was started, so the example only asserts that it is non-empty.

Example:

    test -n "$0" && echo "has a name"
